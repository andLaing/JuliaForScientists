...
# ╔═╡ c10c47ba-06d2-4c56-b2a0-ec59847296e0
using StatsBase
...
# ╔═╡ 1af67e7b-eed3-4d28-a0e4-bc2aa3a05a4f
names(df_dict), propertynames(df_dict)
...
df_direct = DataFrame("x" => Float64[11, 22, 33, 44], "y" => [21, 22, 23, 24], "z" => [31, 32, 33, 34])
...
# ╔═╡ 162008ba-8c06-43ee-826f-80ca8b174bca
df_direct.x === df_direct[:, :x]
...
# ╔═╡ 6954b428-fd7a-4d78-8d39-e472aa90fcfc
df_direct[!, :x], df_direct[!, "x"]
...
# ╔═╡ 2d469bea-9743-4703-aab6-992e4b36c101
DataFrame(rand(Float64, (4,4)), :auto)
...
df_manipulate = let
	...
	df_manipulate.flt_vals = df_manipulate.x * 2.9
	df_manipulate
end
...
# ╔═╡ 591e2000-f254-4f70-b3d7-30ebf5ee2dc7
names(df_iris)
...
combine(df_iris, :PetalWidth, :PetalWidth => sum)

# ╔═╡ 7ed46ec9-67a1-4ddc-a797-a0ca12971cb7
eltype.(eachcol(df_iris))
...
# ╔═╡ 56b2a00c-004f-47bc-b141-394c03b59388
...
let
	...
	combine(grp_species, :PetalWidth => (col -> sum(col) / length(col)) => :NewFunctionStuff, :SepalLength => (col -> sum(col) / length(col)), ungroup=false)
end

# ╔═╡ 8624b7d0-29a4-4eb6-a40c-dca74b23b210
grp_species[keys(grp_species)[2]]

# ╔═╡ 4188c59e-c78d-44d1-adca-5ef8cc824276
md"## _5. Challenge_

Use the data we read from `iris.csv` to calculate the correlation coefficients between the columns for each iris type in DataFrame format. Report and plot distributions of the relevant columns for each of the types of iris."

# ╔═╡ 4401b494-8406-4246-9422-c53ef740bfa5
iris_with_corr = let
	val_cols = valuecols(grp_species)
	col_combs = [[v1, v2] for (i, v1) in enumerate(val_cols[1:end-1]) for v2 in val_cols[i+1:end]]
	transform(grp_species, col_combs .=> cor)
end

# ╔═╡ 5023cd6b-fb21-407c-86b7-e7875f609e38
function combine_plots(edges::Vector{<:Real}, weights::Vector{<:Real}; pl::Union{Nothing,Plots.Plot}=nothing, label::String="blah")::Plots.Plot
	if isnothing(pl)
		return plot(edges, weights, label=label)
	end
	return plot!(pl, edges, weights, label=label)
end
...
begin
	...
	current()
	#plot(pl1)
	# 	p = plot!(p, histogram(grp.PetalWidth, bins=LinRange(extrema(grp.PetalWidth)..., 50)), label=key.Species)
	# end
end
